...
- Динамический фильтр котировок: оборот ≥150M USD и |изменение| ≥5%
...
class Engine:
    ...
    def on_kline_closed_1m(self, sym: str) -> Optional[Dict[str, Any]]:
        ...
        if side == "LONG":
            # своя стена ниже цены (от которой ждём отскок)
            if obf["walls_bid"]:
        ...
        if near_same is not None:
            ...
            # фронт-ран 2–6 тиков от границы кластера
        ...
        # --- Постановка целей/стопов (с учётом плотности)
        entry = c
        # SL: если есть OB-зона от pending — ставим за зону; иначе — за экстремум свечи ±ATR-буфер
        if side == "LONG":
            sl = (ob_zone[0] - 0.05 * atr1) if ob_zone else (rows1[-1][2] - 0.20 * atr1)
        else:
            sl = (ob_zone[1] + 0.05 * atr1) if ob_zone else (rows1[-1][1] + 0.20 * atr1)

        # TP расчет: структура/ATR/таймбокс
        SH1, SL1 = find_swings(rows1, SWING_FRAC)
        if side == "LONG":
            # ближайший прошлый свинг-high выше входа
            tp_struct = None
...
async def universe_refresh_loop(app: web.Application) -> None:
    ...
    while True:
        try:
            ...
            symbols_new: List[str] = [s for s,_ in cands[:ACTIVE_SYMBOLS]]
            symbols_old: List[str] = app.get("symbols", [])
            if not symbols_new:
                symbols_new = symbols_old
            ...
            if add or rem:
                ...
                if add:
                    ...
                    # прогрузим instrument info
                    for s in add:
                        with contextlib.suppress(Exception):
                            info = await rest.instrument_info(s)
                            if info:
                                # разный формат tickSize у Bybit — страхуемся
...
async def ws_on_message(app: web.Application, data: Dict[str, Any]) -> None:
    ...
    if topic.startswith("tickers."):
        ...
        # Bybit v5 L2: { topic: "orderbook.50.SYMBOL", type: "snapshot"/"delta", data: {...} }
        d = data.get("data") or {}
...
async def on_startup(app: web.Application) -> None:
    ...
    # Фильтр котировок (динамический): оборот≥150M и |change|≥5%
    cands: List[Tuple[str, float]] = []
    for t in tickers:
        sym = t.get("symbol")
    ...
    # прогрузим tickSize для выбранных
    for s in symbols:
        with contextlib.suppress(Exception):
            info = await rest.instrument_info(s)
            if info:
